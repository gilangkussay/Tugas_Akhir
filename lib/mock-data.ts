...
for (let i = 6; i <= 25; i++) {
  mockProducts.push({
    ...
    stock: ((i * 7) % 30) + 5,
    category_id: 'laptops',
    images: [laptopImages[(i - 6) % laptopImages.length]],
    rating: 4 + ((i * 3) % 10) / 10,
...
for (let i = 31; i <= 65; i++) {
  mockProducts.push({
    ...
    images: [pcComponentImages[(i - 31) % pcComponentImages.length]],
    rating: 4 + ((i * 5) % 10) / 10,
...
for (let i = 71; i <= 95; i++) {
  mockProducts.push({
    ...
    images: [accessoryImages[(i - 71) % accessoryImages.length]],
    rating: 4 + ((i * 7) % 10) / 10,
